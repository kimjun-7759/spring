...
var replyService = (function() {
	...
	function getList(param, callback, error) {
		...
		$.getJSON("/replies/pages/" + board_idx + "/" + page + ".json", function(data) {
			if (callback) {
...
//	}
	...
	*/
	...
	// 댓글 삭제. security 적용 후
	function remove(comment_idx, replyer, callback, error) {
		  
		console.log("--------------------------------------");  
		console.log(JSON.stringify({comment_idx:comment_idx, replyer:replyer}));  
		    
		$.ajax({
			type : 'delete',
			url : '/replies/' + comment_idx,
			data:  JSON.stringify({comment_idx:comment_idx, replyer:replyer}),
			contentType: "application/json; charset=utf-8",
			success : function(deleteResult, status, xhr) {
				if (callback) {
					callback(deleteResult);
	...
	function update(reply, callback, error) {
		...
		console.log("comment_idx: " + reply.comment_idx);

		$.ajax({
			type : 'put',
			url : '/replies/' + reply.comment_idx,
			data : JSON.stringify(reply), // JavaScript 값이나 객체를 JSON 문자열로 변환
			contentType : "application/json; charset=utf-8",
			success : function(result, status, xhr) {
				if (callback) {
